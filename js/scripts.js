...
const calculateAverage = numbers => {
	...
	for (const number of numbers) {
		...
		additionResult = additionResult + number; // /numbers.length > por qué no sale bien la división dentro?;
	}
...
const getSingleString = arrayOfStrings => {
	...
	for (const word of arrayOfStrings) {
		if (vowels.includes(word)) {
			singleString = singleString + word;
		}
	}
...
const getLargeNames = nameList => {
	...
	for (const name of nameList) {
		if (name.length > 5) {
			namePosition.push(name.indexOf()); //se que está mal
		}
	}
